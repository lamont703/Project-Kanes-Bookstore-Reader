...
- **Book upload/processing**: `components/admin/book-form.tsx` calls the Supabase Edge Function `upload-book` directly (`${NEXT_PUBLIC_SUPABASE_URL}/functions/v1/upload-book`, Deno + `supabase/functions/upload-book/`). That function then **delegates the PDF work back to Next.js**: it POSTs to `${VERCEL_PROJECT_URL}/api/admin/process-book` with `Authorization: Bearer ${INTERNAL_API_SECRET}` (see `supabase/functions/upload-book/handler.ts`). So `app/api/admin/process-book/route.ts` (Node + `mupdf` via `lib/book/pdf-batch-processor.ts`) is **live infrastructure on every book upload**, not legacy — it is simply never called from the browser. That handshake is why `INTERNAL_API_SECRET` exists and why `VERCEL_PROJECT_URL` must point at the same environment you are uploading from.
